...
const fragmentShader = glsl`
  ...
  void main() {  
    ...
    // vec2 newBlendedUv = vUv + 0.01 * cnoise(10. * vUv + time) - 0.5;
    vec2 newBlendedUv = vUv + uNoiseAmplitude * cnoise(uNoiseFrequency * vUv + time) - 0.5;
    vec2 scaledUV = newBlendedUv * sin(scaleFactor + cnoise(1. * newBlendedUv + time * 1.)) + 0.5;
    ...
    blendedColor2 += texture2D(uTexture2, scaledUV + vec2(offsetAmount, offsetAmount));
    blendedColor2 += texture2D(uTexture2, scaledUV + vec2(-offsetAmount, -offsetAmount));
    blendedColor2 += texture2D(uTexture2, scaledUV + vec2(offsetAmount, -offsetAmount));
    blendedColor2 += texture2D(uTexture2, scaledUV + vec2(-offsetAmount, offsetAmount));
    blendedColor2 += texture2D(uTexture2, scaledUV);
